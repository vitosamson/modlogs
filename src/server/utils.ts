...
const getPeriodAmountAndUnit = (period: string): { amount: number; unit: string; } => {
  const amount = parseInt((period.match(/\d+/) || [])[0], 10) || 1;
  const unit = ((period.match(/\D+/) || [])[0] || 'month').trim();
  return { amount, unit };
};

export function parsePeriod(period: string, maxPeriod?: string): { periodTimestamp: number; humanizedPeriod: string; } {
  const today = moment().utc().startOf('day');

  // tslint:disable-next-line:prefer-const
  let { amount: periodAmount, unit: periodUnit } = getPeriodAmountAndUnit(period);
  ...
  let duration = moment.duration(periodAmount, periodUnit as moment.unitOfTime.DurationConstructor);

  if (maxPeriod) {
    const { amount: maxAmount, unit: maxUnit } = getPeriodAmountAndUnit(maxPeriod);
    const maxDuration = moment.duration(maxAmount, maxUnit as moment.unitOfTime.DurationConstructor);
