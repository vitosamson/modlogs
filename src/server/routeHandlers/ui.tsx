...
const clientAssets = JSON.parse(
  readFileSync(resolve('build/client/assets.json')).toString()
) as HtmlProps;
...
function loadProps(renderProps: any, store: Store<any>): Promise<LoadedProps> {
  return new Promise<LoadedProps>((resolve, reject) => {
    loadPropsOnServer(
      renderProps,
      { dispatch: store.dispatch, location: renderProps.location },
      (err, asyncProps, scriptTag) => {
