...
describe('parsePeriod', () => {
  it('returns the correct unix timestamp from the period string', () => {
    const oneDayAgo = moment().utc().startOf('day').subtract(1, 'day').unix() * 1000;
    expect(parsePeriod('1 day').periodTimestamp).toEqual(oneDayAgo);
    expect(parsePeriod('1 d').periodTimestamp).toEqual(oneDayAgo);
    expect(parsePeriod('1 D').periodTimestamp).toEqual(oneDayAgo);

    const fourMonthsAgo = moment().utc().startOf('day').subtract(1, 'month').unix() * 1000;
  ...
  it('honors maxPeriod', () => {
    const twoWeeksAgo = moment().utc().startOf('day').subtract(2, 'week').unix() * 1000;
    const { periodTimestamp, humanizedPeriod } = parsePeriod('4 weeks', '2 weeks');
