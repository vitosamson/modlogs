...
describe('createMongoProjectionFromConfig', () => {
  ...
  const doQuery = (c: ISubredditModlogConfig): Promise<ILog[]> =>
    collection
      .aggregate([{ $project: createMongoProjectionFromConfig(c) }])
      .toArray();
  const createFilter = (by: keyof ILog) => (logs: ILog[]) =>
    logs.filter(l => !!l[by]);
  ...
  it('automod action reasons', async () => {
    ...
    expect(logs[0].automodActionReason).toEqual(
      'Auto-remove text posts with no body'
    );
